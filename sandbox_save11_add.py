...
class Sandbox:
    ...
    def simulate_step(self):
            ...
            for y, x in np.argwhere(insect_mask):
                ...
                if 0 <= nx < self.width and 0 <= ny < self.height:  # 确保目标合法
                    ...
                    if self.layers["Crops"][ny, nx] == 1:
                        energy += self.crop_energies[ny, nx] * self.config["insect_crop_energy_gain_ratio"]
                        self.crop_energies[ny, nx] *= 0.6
        ...
        # 记录统计数据
        self.stats.append(
            (self.current_time_step, crop_count, insect_count, bird_count, bat_count, harvest_energy_ratio, avg_human_activity_fertility)
        )
    ...
    def save_results(self):
        ...
        df = pd.DataFrame(
            self.stats,
            columns=["TimeStep", "Crops", "Insects", "Birds", "Bats", "HarvestEnergyRatio", "AvgHumanActivityFertility"]
        )
    ...
    def plot_stats(self):
        # 提取统计数据
        timesteps, crops, insects, birds, bats, harvest_energy_ratio, avg_human_activity_fertility = zip(*self.stats)

        # 创建第一个窗口：显示动物单位数量
        plt.figure(figsize=(12, 6))
        plt.plot(timesteps, insects, label="Insects", linestyle='-', marker='o', color='yellow')
        plt.plot(timesteps, birds, label="Birds", linestyle='-', marker='s', color='purple')
        plt.plot(timesteps, bats, label="Bats", linestyle='-', marker='^', color='blue')
...
if __name__ == "__main__":
    sandbox = Sandbox(config_file="config_save11_add.json")
    sandbox.render()
