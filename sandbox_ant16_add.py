...
class Sandbox:
    def __init__(self, config_file="config.json"):
        ...
        self.layers = {
            ...
            "Ants": np.zeros((self.height, self.width), dtype=int),  # 切叶蚁图层
            "AntNests": np.zeros((self.height, self.width), dtype=int),  # 巢穴图层
        ...
        # 初始化切叶蚁相关图层和变量
        self.ant_energies = np.full((self.height, self.width), -1, dtype=int)  # 切叶蚁能量
        self.ant_ages = np.zeros((self.height, self.width), dtype=int)  # 切叶蚁寿命
        self.ant_load = np.zeros((self.height, self.width), dtype=bool)  # 是否处于load状态
        ...
        self.colors = {
            ...
            "Worm": (20, 20, 20), # 蚯蚓颜色（灰黑色）
            "AntNest": (255, 255, 255), # 巢穴颜色（白色）
            "Ant": (0, 255, 0)  # 正常蚂蚁用绿色
        }
    ...
    def simulate_step(self):
        ...
        # 切叶蚁巢穴更新逻辑
        nest_mask = self.layers["AntNests"] == 1
        if np.any(nest_mask):
            for y, x in np.argwhere(nest_mask):
                # 根据季节调整蚂蚁生成概率
                if self.season == 1:  # 夏季，生成概率较高
                    spawn_chance = self.config["ant_spawn_chance_summer"]
                    spawn_count = np.random.randint(1, self.config["ant_spawn_count_summer"] + 1)
                elif self.season == 3:  # 冬季，生成概率较低
                    spawn_chance = self.config["ant_spawn_chance_winter"]
                    spawn_count = 0  # 冬天不生成蚂蚁
                else:  # 春季和秋季
                    spawn_chance = self.config["ant_spawn_chance"]
                    spawn_count = np.random.randint(1, self.config["ant_spawn_count"] + 1)
                ...
                if np.random.rand() < spawn_chance:
                    for _ in range(spawn_count):
                        potential_positions = [
                            ...
                            self.layers["Ants"][ny, nx] == 0 and  # 无蚂蚁
                            self.layers["Seedlings"][ny, nx] == 0 and      # 无树苗
                            self.layers["MatureTrees"][ny, nx] == 0        # 无树叶或树干
                        ]
                        if potential_positions:
                            ax, ay = potential_positions[np.random.randint(len(potential_positions))]
                            self.layers["Ants"][ay, ax] = 1
                            self.ant_energies[ay, ax] = self.config["ant_initial_energy"]
                            self.ant_ages[ay, ax] = 0
                            self.ant_load[ay, ax] = False

        # 切叶蚁更新逻辑
        ant_mask = self.layers["Ants"] > 0  # 切叶蚁单位
        if np.any(ant_mask):
            for y, x in np.argwhere(ant_mask):
                energy = self.ant_energies[y, x]
                age = self.ant_ages[y, x]

                # 增加年龄
                age += 1
                self.ant_ages[y, x] = age

                # 检查寿命限制
                if age > self.config["ant_lifetime"]:
                    self.layers["Ants"][y, x] = 0  # 蚂蚁死亡
                    self.layers["Land"][y, x] += int(energy * self.config["ant_fertility_return_ratio"])
                    continue

                # 消耗能量
                energy -= self.config["ant_energy_loss_per_move"]

                # 移动逻辑
                if self.ant_load[y, x]:  # 处于load状态
                    # 寻找最近巢穴
                    nest_positions = np.argwhere(self.layers["AntNests"] == 1)
                    if len(nest_positions) > 0:
                        target = nest_positions[np.argmin(np.sum(np.abs(nest_positions - [y, x]), axis=1))]
                        dx, dy = np.sign(target[1] - x), np.sign(target[0] - y)
                    else:
                        dx, dy = np.random.choice([-1, 0, 1]), np.random.choice([-1, 0, 1])
                    ...
                    if 0 <= nx < self.width and 0 <= ny < self.height:
                        if self.layers["AntNests"][ny, nx] == 1:  # 返回巢穴
                            self.layers["Land"][ny - 3:ny + 4, nx - 3:nx + 4] += self.config["ant_nest_fertility_boost"]
                            self.ant_load[y, x] = False  # 重置load状态
                            energy = self.config["ant_initial_energy"]  # 补满能量
                        else:
                            self.layers["Ants"][ny, nx] = 1
                            self.ant_energies[ny, nx] = energy
                            self.layers["Ants"][y, x] = 0

                else:  # 未load状态
                    # 寻找最近树苗或树叶
                    plant_positions = np.argwhere((self.layers["Seedlings"] == 1) | (self.layers["MatureTrees"] == 2))
                    if len(plant_positions) > 0:
                        target = plant_positions[np.argmin(np.sum(np.abs(plant_positions - [y, x]), axis=1))]
                        dx, dy = np.sign(target[1] - x), np.sign(target[0] - y)
                    else:
                        dx, dy = np.random.choice([-1, 0, 1]), np.random.choice([-1, 0, 1])
                    ...
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < self.width and 0 <= ny < self.height:
                        if self.layers["Seedlings"][ny, nx] == 1:
                            self.layers["Seedlings"][ny, nx] = 0  # 移除树苗
                            self.ant_load[y, x] = True  # 设置为load状态
                        elif self.layers["MatureTrees"][ny, nx] == 2:
                            if np.random.rand() < self.config["ant_leaf_clear_chance"]:
                                self.layers["MatureTrees"][ny, nx] = 0  # 移除树叶
                                self.ant_load[y, x] = True  # 设置为load状态
                        else:
                            self.layers["Ants"][ny, nx] = 1
                            self.ant_energies[ny, nx] = energy
                ...
                if self.season == 0:  # 春天
                    for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        ...
                        if 0 <= ny < self.height and 0 <= nx < self.width:
                            if self.layers["Crops"][ny, nx] == 1:
                                self.crop_energies[ny, nx] = min(
                                    self.crop_energies[ny, nx] + self.config["ant_pollination_boost"],
                                    self.config["crop_max_energy"]
                                )
                ...
                for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    ny, nx = y + dy, x + dx
                    if 0 <= ny < self.height and 0 <= nx < self.width:
                        if self.layers["Insects"][ny, nx] == 1:
                            insect_energy = self.insect_energies[ny, nx]
                            energy = min(
                                self.config["ant_max_energy"],
                                energy + insect_energy
                            )
                            self.layers["Land"][ny, nx] += int(insect_energy * self.config["ant_insect_fertility_return"])
                            self.layers["Insects"][ny, nx] = 0

                # 更新蚂蚁状态和能量
                if energy <= 0:
                    self.layers["Ants"][y, x] = 0  # 蚂蚁死亡
                else:
                    self.ant_energies[y, x] = energy
        ...
        ant_count = np.count_nonzero(self.layers["Ants"])
    ...
    def save_results(self):
        ...
        df = pd.DataFrame(
            self.stats,
            columns=["TimeStep", "Crops", "Insects", "Birds", "Bats", "Bees", "Worms", "Ants" "HarvestEnergyRatio", "AvgHumanActivityFertility"]
        )
    ...
    def plot_stats(self):
        ...
        plt.plot(timesteps, worms, label="Worms", linestyle='-', marker='v', color='grey')
        plt.plot(timesteps, worms, label="Worms", linestyle='-', marker='x', color='green')
    ...
    def save_state(self):
        ...
        state = {
            ...
            "timers": {
                ...
                "bat_ages": self.bat_ages.tolist(),  # 保存蝙蝠的年龄信息
                "bee_ages": self.bee_ages.tolist(),  # 保存蜜蜂的年龄信息
                "ant_ages": self.ant_ages.tolist(),  # 保存蚂蚁的年龄信息
            },
            "energies": {
                ...
                "worm_energies": self.worm_energies.tolist(),
                "bee_energies": self.bee_energies.tolist(),
                "ant_energies": self.ant_energies.tolist(),  # 保存蚂蚁的能量信息
            },
            "season": self.season,
    ...
    def load_state(self, file_path):
        ...
        # self.bat_ages = np.array(state["timers"]["bat_ages"], dtype=int)
        # self.bee_ages = np.array(state["timers"]["bee_ages"], dtype=int)
        # self.ant_ages = np.array(state["timers"]["ant_ages"], dtype=int)
        ...
        # self.worm_energies = np.array(state["energies"]["worm_energies"], dtype=int)
        # self.bee_energies = np.array(state["energies"]["bee_energies"], dtype=int)
        # self.ant_energies = np.array(state["energies"]["ant_energies"], dtype=int)  # 恢复蚂蚁能量
    ...
    def render(self):
        ...
        """
        pygame.init()
        screen = pygame.display.set_mode((self.width * self.cell_size + 400, self.height * self.cell_size + 150))
        ...
        while running:
            ...
            ant_nest_layer = self.layer_visibility["AntNests"]
            ant_layer = self.layer_visibility["Ants"]
            ...
            for y in range(self.height):
                for x in range(self.width):
                    ...
                    if ant_layer:
                        if self.layers["Ants"][y, x] == 1:
                            pygame.draw.circle(
                                screen, self.colors["Ant"],
                                (x * self.cell_size + self.cell_size // 2, y * self.cell_size + self.cell_size // 2),
                                self.cell_size // 3
                            )
                    
                    if ant_nest_layer:
                        if self.layers["AntNests"][y, x] == 1:
                            pygame.draw.rect(
                                screen, self.colors["AntNest"],
                                pygame.Rect(x * self.cell_size, y * self.cell_size, self.cell_size, self.cell_size)
                            )
            ...
            pygame.draw.rect(screen, (50, 50, 50), (0, toolbar_y, self.width * self.cell_size + 400, 50))
            ...
            ant_nest_button = pygame.Rect(820, toolbar_y + 10, 80, 30)
            herbicide_button = pygame.Rect(920, toolbar_y + 10, 80, 30)
            insecticide_button = pygame.Rect(1020, toolbar_y + 10, 80, 30)
            delete_button = pygame.Rect(1120, toolbar_y + 10, 80, 30)
            ...
            pygame.draw.rect(screen, (255, 255, 255), ant_nest_button)  # 白色按钮
            ...
            screen.blit(font.render("Ant Nest", True, (0, 0, 0)), (825, toolbar_y + 15))
            screen.blit(font.render("PES-her", True, (0, 0, 0)), (925, toolbar_y + 15))
            screen.blit(font.render("PES-ins", True, (0, 0, 0)), (1025, toolbar_y + 15))
            screen.blit(font.render("Delete", True, (0, 0, 0)), (1125, toolbar_y + 15))
            ...
            pygame.draw.rect(screen, (0, 0, 0), (0, info_bar_y, self.width * self.cell_size + 400, info_bar_height))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        self.layer_visibility["Ants"] = not self.layer_visibility["Ants"]
                        self.layer_visibility["AntNests"] = not self.layer_visibility["AntNests"]
                    ...
                    elif event.key == pygame.K_t:  # 读取状态
                    ...
                    if toolbar_y <= mouse_pos[1] < toolbar_y + 50:
                        if seedling_button.collidepoint(mouse_pos):
                            ...
                            self.brush = "Worm"
                        elif ant_nest_button.collidepoint(mouse_pos):
                             self.brush = "AntNest"
                    ...
                    if pygame.mouse.get_pressed()[0]:  # Left mouse button
                        if 0 <= grid_x < self.width and 0 <= grid_y < self.height:
                            ...
                            elif self.brush == "AntNest":
                                if self.layers["AntNests"][grid_y, grid_x] == 0:  # 确保位置未被占用
                                    self.layers["AntNests"][grid_y, grid_x] = 1  # 放置巢穴
...
if __name__ == "__main__":
    sandbox = Sandbox(config_file="config_ant16_add.json")
    sandbox.render()
