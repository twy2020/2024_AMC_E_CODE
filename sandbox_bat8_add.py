...
class Sandbox:
    def __init__(self, config_file="config.json"):
        ...
        self.layers["Bats"] = np.zeros((self.height, self.width), dtype=int)  # 蝙蝠图层
    ...
    def simulate_step(self):
        ...
        if np.any(bird_mask):
            for y, x in np.argwhere(bird_mask):
                ...
                # 随机移动逻辑
                if energy > self.config["bird_reproduction_chance"]:  # 高能量状态：50%捕食，50%繁殖
                    if np.random.rand() < 0.5:  # 捕食行为
                        ...
                        if 0 <= nx < self.width and 0 <= ny < self.height:  # 确保移动合法
                            ...
                            if self.layers["MatureTrees"][ny, nx] in [1, 2]:
                                if np.random.rand() < self.config["bird_reproduction_chance"]:
                                    potential_spots = [
                                        (nx + dx, ny + dy)
                                        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]
        ...
        if np.any(bat_mask):
            for y, x in np.argwhere(bat_mask):
                ...
                if 0 <= nx < self.width and 0 <= ny < self.height:  # 确保目标合法
                    ...
                    # 蝙蝠繁殖逻辑
                    if energy > 1000 and np.random.rand() < self.config["bat_reproduction_chance"]:
                        tree_positions = np.argwhere(
                            (self.layers["MatureTrees"] == 1) | (self.layers["MatureTrees"] == 2)
                        )  # 树干或树叶
                        potential_positions = [
    ...
    def render(self):
        ...
        while running:
            ...
            delete_button = pygame.Rect(620, toolbar_y + 10, 80, 30)
            ...
            screen.blit(font.render("Delete", True, (0, 0, 0)), (625, toolbar_y + 15))
...
if __name__ == "__main__":
    sandbox = Sandbox(config_file="config_bat8_add.json")
    sandbox.render()
