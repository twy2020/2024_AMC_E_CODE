...
class Sandbox:
    ...
    def simulate_step(self):
        ...
        trunk_count = np.count_nonzero(self.layers["MatureTrees"] == 1)  # 树干
        leaf_count = np.count_nonzero(self.layers["MatureTrees"] == 2)  # 树叶
        avg_fertility = np.sum(self.layers["Land"]) / (self.width * self.height)

        # 记录统计数据
        self.stats.append((self.current_time_step, seedling_count, trunk_count, leaf_count, avg_fertility))
    ...
    def save_results(self):
        ...
        # 保存Excel文件
        df = pd.DataFrame(self.stats, columns=["TimeStep", "Seedlings", "TreeTrunks", "Leaves", "AvgFertility"])
