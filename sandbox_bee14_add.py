...
class Sandbox:
    ...
    def simulate_step(self):
        ...
        if self.season == 0 and np.random.rand() < self.config["spring_bird_spawn_chance"]:
            ...
            if len(spawn_positions) > 0:
                ny, nx = spawn_positions[np.random.randint(len(spawn_positions))]
                if self.layers["Birds"][ny, nx] == 0 and self.layers["Bats"][ny, nx] and self.layers["Bees"][ny, nx] == 0:  # 确保目标格子没有鸟
        ...
        # 记录统计数据
        self.stats.append(
            (self.current_time_step, crop_count, insect_count, bird_count, bat_count, bee_count, harvest_energy_ratio, avg_human_activity_fertility)
        )
    ...
    def save_results(self):
        ...
        df = pd.DataFrame(
            self.stats,
            columns=["TimeStep", "Crops", "Insects", "Birds", "Bats", "Bees", "HarvestEnergyRatio", "AvgHumanActivityFertility"]
        )
    ...
    def plot_stats(self):
        # 提取统计数据
        timesteps, crops, insects, birds, bats, bees, harvest_energy_ratio, avg_human_activity_fertility = zip(*self.stats)
    ...
    def render(self):
        ...
        while running:
            ...
            herbicide_button = pygame.Rect(720, toolbar_y + 10, 80, 30)
            insecticide_button = pygame.Rect(820, toolbar_y + 10, 80, 30)
            delete_button = pygame.Rect(920, toolbar_y + 10, 80, 30)
            ...
            screen.blit(font.render("PES-her", True, (0, 0, 0)), (725, toolbar_y + 15))
            screen.blit(font.render("PES-ins", True, (0, 0, 0)), (825, toolbar_y + 15))
            screen.blit(font.render("Delete", True, (0, 0, 0)), (925, toolbar_y + 15))
...
if __name__ == "__main__":
    sandbox = Sandbox(config_file="config_bee14_add.json")
    sandbox.render()
