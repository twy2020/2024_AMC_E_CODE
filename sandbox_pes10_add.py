...
class Sandbox:
    ...
    def simulate_step(self):
        ...
        if np.any(insect_mask):
            for y, x in np.argwhere(insect_mask):
                ...
                if self.season == 3 and np.random.rand() < self.config["winter_death_chance"] * 5:
                ...
                if 0 <= nx < self.width and 0 <= ny < self.height:  # 确保目标合法
                    ...
                    if self.layers["Crops"][ny, nx] == 1:
                        self.crop_energies[ny, nx] = max(0, self.crop_energies[ny, nx] - 40)
                    ...
                    if energy > 200 and np.random.rand() < insect_reproduction_chance:
                        # 获取周围邻居的空位
                        potential_positions = [
                            (nx + dx, ny + dy)
                            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]
                    ...
                    if energy < self.config["insect_death_energy_threshold"]:
                        ...
                        self.layers["Land"][y, x] = min(
                            self.config["land_default_fertility"], self.layers["Land"][y, x] + self.config["insect_fertility_return"]
                        )
                    else:
                        # 更新昆虫位置和能量
                        self.layers["Insects"][ny, nx] = 1
        ...
        if np.any(bird_mask):
            for y, x in np.argwhere(bird_mask):
                ...
                if energy > 1000 and np.random.rand() < bird_reproduction_chance:
                    if np.random.rand() < 0.5:  # 捕食行为
                        if np.random.rand() < 0.9:  # 90% 概率捕食昆虫
                        ...
                        else:  # 如果区域内无树叶或树干，随机移动
                            dx, dy = np.random.choice([-2, -1, 0, 1, 2]), np.random.choice([-2, -1, 0, 1, 2])

                        # 执行繁殖逻辑
                        nx, ny = x + dx, y + dy
                        if 0 <= nx < self.width and 0 <= ny < self.height:  # 确保移动合法
                            energy -= self.config["bird_energy_loss_per_move"]
                            if self.layers["MatureTrees"][ny, nx] in [1, 2]:
                                potential_spots = [
                                    (nx + dx, ny + dy)
                                    for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]
                                    if 0 <= nx + dx < self.width and 0 <= ny + dy < self.height
                                    and self.layers["Birds"][ny + dy, nx + dx] == 0
                                ]
                                if potential_spots:
                                    bx, by = potential_spots[np.random.randint(len(potential_spots))]
                                    self.layers["Birds"][by, bx] = 1
                                    self.bird_energies[by, bx] = self.config["bird_initial_energy"]
                                    self.bird_ages[by, bx] = 0

                else:  # 普通捕食逻辑
                    if np.random.rand() < 0.9:  # 90% 捕食昆虫
                        insect_positions = np.argwhere(self.layers["Insects"] == 1)
                        if len(insect_positions) > 0:
                            target = insect_positions[np.random.randint(len(insect_positions))]
                            dx, dy = np.sign(target[1] - x), np.sign(target[0] - y)
        ...
        self.stats.append((self.current_time_step, seedling_count, trunk_count, avg_fertility, crop_count, insect_count, bird_count, self.crop_harvest_energy))
    ...
    def save_results(self):
        ...
        # 保存Excel文件
        df = pd.DataFrame(self.stats, columns=["TimeStep", "Seedlings", "TreeTrunks", "AvgFertility", "Crops", "Insects", "Birds", "HarvestEnergy"])
    ...
    def plot_stats(self):
        # 提取统计数据
        timesteps, seedlings, trunks, avg_fertility, crops, insects, birds, harvest_energy = zip(*self.stats)
        ...
        plt.plot(timesteps, crops, label="Crops", linestyle='-', marker='^', color='orange')
        plt.plot(timesteps, insects, label="Insects", linestyle='-', marker='o', color='yellow')
        plt.plot(timesteps, birds, label="Birds", linestyle='-', marker='o', color='purple')
        ...
        # 创建一个新的窗口绘制农作物平均采收能量和土地平均肥力
        plt.figure(figsize=(12, 8))
        plt.plot(timesteps, harvest_energy, label="Harvest Energy", linestyle='-', marker='o', color='red')
        plt.plot(timesteps, avg_fertility, label="Avg Fertility", linestyle='-', marker='x', color='blue')

        plt.xlabel("Time Step")
        plt.ylabel("Energy / Avg Fertility")
        plt.title("Harvest Energy and Avg Fertility Over Time")
        plt.legend()
        plt.grid(True)
        plt.show()
    ...
    def render(self):
        ...
        while running:
            ...
            herbicide_button = pygame.Rect(620, toolbar_y + 10, 80, 30)
            insecticide_button = pygame.Rect(720, toolbar_y + 10, 80, 30)
            delete_button = pygame.Rect(820, toolbar_y + 10, 80, 30)
            ...
            pygame.draw.rect(screen, (255, 0, 255), bat_button)  # 蓝色表示鸟单位
            ...
            screen.blit(font.render("Bird", True, (255, 255, 255)), (425, toolbar_y + 15))
            screen.blit(font.render("Bat", True, (0, 0, 0)), (525, toolbar_y + 15))
            screen.blit(font.render("Herbicide", True, (0, 0, 0)), (625, toolbar_y + 15))
            screen.blit(font.render("Insecticide", True, (0, 0, 0)), (725, toolbar_y + 15))
            screen.blit(font.render("Delete", True, (0, 0, 0)), (825, toolbar_y + 15))
...
if __name__ == "__main__":
    sandbox = Sandbox(config_file="config_pes10_add.json")
    sandbox.render()
